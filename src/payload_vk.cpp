...
static bool make_image(DeviceCtx *dc, VkImage *img, VkDeviceMemory *mem, VkImageView *view,
    ...
    if (ma.memoryTypeIndex == UINT32_MAX || f.AllocateMemory(dc->dev, &ma, nullptr, mem) != VK_SUCCESS)
        return false;
    f.BindImageMemory(dc->dev, *img, *mem, 0);
...
static bool make_buffer(DeviceCtx *dc, VkBuffer *buf, VkDeviceMemory *mem, VkDeviceSize size) {
    ...
    ma.allocationSize = mr.size;
    ma.memoryTypeIndex = find_memory_type(dc, mr.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    if (ma.memoryTypeIndex == UINT32_MAX || f.AllocateMemory(dc->dev, &ma, nullptr, mem) != VK_SUCCESS) return false;
    f.BindBufferMemory(dc->dev, *buf, *mem, 0);
    return true;
}
...
static bool create_swapchain_resources(SwapchainCtx *cx) {
    ...
    if (f.CreateGraphicsPipelines(dc->dev, VK_NULL_HANDLE, 1, &gp, nullptr, &cx->pipeline) != VK_SUCCESS) { VKLOGE("init[6/9] CreateGraphicsPipelines 失败"); return false; }
    ...
    if (!vkcfg.vk_mci) {
        ...
        if (blit_src && blit_dst && stor_ok) {
            ...
            if (f.CreateComputePipelines(dc->dev, VK_NULL_HANDLE, 1, &cpi0, nullptr, &cx->cs0_pipeline) != VK_SUCCESS) {
                if (cx->cs0f) { VKLOGI("init[8d/9] fp16 cs0 管线失败，回退 fp32"); cpi0.stage.module = cx->cs0; }
                if (f.CreateComputePipelines(dc->dev, VK_NULL_HANDLE, 1, &cpi0, nullptr, &cx->cs0_pipeline) != VK_SUCCESS) { VKLOGE("init[8d/9] CreateComputePipelines(cs0) 失败"); return false; }
            }
            ...
            if (f.CreateComputePipelines(dc->dev, VK_NULL_HANDLE, 1, &cpi1, nullptr, &cx->cs1_pipeline) != VK_SUCCESS) {
                if (cx->cs1f) { VKLOGI("init[8d/9] fp16 cs1 管线失败，回退 fp32"); cpi1.stage.module = cx->cs1; }
                if (f.CreateComputePipelines(dc->dev, VK_NULL_HANDLE, 1, &cpi1, nullptr, &cx->cs1_pipeline) != VK_SUCCESS) { VKLOGE("init[8d/9] CreateComputePipelines(cs1) 失败"); return false; }
            }
            ...
            if (f.CreateComputePipelines(dc->dev, VK_NULL_HANDLE, 1, &cpiG0, nullptr, &cx->gmv0_pipeline) != VK_SUCCESS) { VKLOGE("init[8d/9] CreateComputePipelines(gmv0) 失败"); return false; }
            ...
            if (f.CreateComputePipelines(dc->dev, VK_NULL_HANDLE, 1, &cpiG1, nullptr, &cx->gmv1_pipeline) != VK_SUCCESS) { VKLOGE("init[8d/9] CreateComputePipelines(gmv1) 失败"); return false; }
            ...
            if (cx->l2_ok) {
                ...
                if (f.CreateComputePipelines(dc->dev, VK_NULL_HANDLE, 1, &cpi0l2, nullptr, &cx->cs0_l2_pipeline)!=VK_SUCCESS) {
                    ...
                    f.CreateComputePipelines(dc->dev, VK_NULL_HANDLE, 1, &cpi0l2, nullptr, &cx->cs0_l2_pipeline);
            ...
            if (cx->l2_ok) {
                ...
                if (f.CreateComputePipelines(dc->dev, VK_NULL_HANDLE, 1, &cpi1l2, nullptr, &cx->cs1_l2_pipeline)!=VK_SUCCESS) {
                    ...
                    f.CreateComputePipelines(dc->dev, VK_NULL_HANDLE, 1, &cpi1l2, nullptr, &cx->cs1_l2_pipeline);
...
static void process_pending_frame(PendingFrame &pf, const McfiConfig &cfg) {
    ...
    image_barrier(cmd, cx->images[idx],
                  VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                  VK_ACCESS_MEMORY_READ_BIT, VK_ACCESS_TRANSFER_READ_BIT,
                  VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);
    image_barrier(cmd, cx->copy_img[cur],
    ...
    if (do_mci) {
        ...
        image_barrier(cmd, cx->copy_img[cur],
                      ...
                      VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);
        image_barrier(cmd, cx->ds_img[cur],
        ...
        if (!cx->ds_layout_init[1 - cur]) {
            image_barrier(cmd, cx->copy_img[1 - cur],
                          VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                          VK_ACCESS_SHADER_READ_BIT, VK_ACCESS_TRANSFER_READ_BIT,
                          VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);
            image_barrier(cmd, cx->ds_img[1 - cur],
        ...
        if (cx->l2_ok) {
            image_barrier(cmd, cx->mv_img_l2[cur],
        ...
        if (cx->gmv0_pipeline && cx->gmv1_pipeline) {
            ...
            bmb.buffer = cx->hist_buf; bmb.offset = 0; bmb.size = VK_WHOLE_SIZE;
            bmb.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
            bmb.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
            f.CmdPipelineBarrier(cmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
            ...
            bmb.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
            f.CmdPipelineBarrier(cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                                 0, 0, nullptr, 1, &bmb, 0, nullptr);
            // gmv image → GENERAL（gmv1 写；首帧 UNDEFINED）
            image_barrier(cmd, cx->gmv_img,
    ...
    if (do_insert) {
        image_barrier(cmd, cx->images[gen_idx],
    ...
    si.signalSemaphoreCount = do_insert ? 1 : 0;
    si.pSignalSemaphores = do_insert ? &slot->done_sem : nullptr;
    if (f.QueueSubmit(queue, 1, &si, slot->fence) == VK_SUCCESS) {
        if (do_insert) {
...
static VkResult VKAPI_CALL my_CreateDevice(VkPhysicalDevice pd, const VkDeviceCreateInfo *pInfo,
    ...
    if (g_inst != VK_NULL_HANDLE) {
        ...
        if (pGetProps2) {
            ...
            VKLOGI("subgroup: arithmetic=%d subgroupSize=%u", (int)g_vk_subgroup, sg.subgroupSize);
        }
    }
