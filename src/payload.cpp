...
static void mcfi_vsync_sample(int64_t now_ns) {
    // vsync_hz>0：手动指定刷新率，跳过自动估计（硬编码 120Hz 在 60/90/144Hz 设备上会设错时间戳 → 掉帧/黑屏）
    if (g_cfg.vsync_hz > 0) {
        g_vsync_period_ns.store(1000000000LL / g_cfg.vsync_hz);
        return;
    }
...
// ---------------- GLES program binary 缓存（GL_OES_get_program_binary / ES3 核心 glGetProgramBinary） ----------------
// 把 compute 程序编译结果缓存到 app files 目录，跨启动跳过 编译+链接（对应 Vulkan 侧 VkPipelineCache）。
// binary 格式与驱动绑定：加载后查 LINK_STATUS，失效自动重链；任何一步失败静默退回正常编译。
static uint32_t gles_src_hash(const char *s) {
    uint32_t h = 5381;
    if (!s) return h;
    while (*s) h = (h * 33) ^ (uint8_t)*s++;
    return h;
}
static std::string gles_prog_path() {
    if (g_pkg.empty()) return "";
    return "/data/user/0/" + g_pkg + "/files/mcfi_gles_prog.bin";
}
struct ProgBinEntry {
    uint32_t hash = 0, fmt = 0, len = 0;
    std::vector<uint8_t> data;
};
// 文件格式：[8B magic][u32 count] { [u32 hash][u32 fmt][u32 len][bytes] }*
static bool gles_prog_load(std::map<uint32_t, ProgBinEntry> &out) {
    out.clear();
    std::string p = gles_prog_path();
    ...
    close(fd);
    if (buf.size() < 12 || memcmp(buf.data(), "MCFI_GLPB", 8) != 0) return false;
    const uint8_t *d = buf.data();
    uint32_t cnt = 0;
    memcpy(&cnt, d + 8, 4);
    size_t off = 12;
    for (uint32_t i = 0; i < cnt && off + 12 <= buf.size(); i++) {
        ProgBinEntry e;
        memcpy(&e.hash, d + off, 4);
        memcpy(&e.fmt, d + off + 4, 4);
        memcpy(&e.len, d + off + 8, 4);
        off += 12;
        if (e.len == 0 || e.len > (8u << 20) || off + e.len > buf.size()) break;
        e.data.assign(d + off, d + off + e.len);
        off += e.len;
        out[e.hash] = std::move(e);
    }
    return true;
}
static void gles_prog_save(const std::map<uint32_t, ProgBinEntry> &entries) {
    std::string p = gles_prog_path();
    if (p.empty() || entries.empty()) return;
    std::string dir = "/data/user/0/" + g_pkg + "/files";
    mkdir(dir.c_str(), 0700);
    int fd = open(p.c_str(), O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0600);
    if (fd < 0) return;
    std::vector<uint8_t> out;
    out.reserve(12 + entries.size() * 24);
    out.insert(out.end(), (const uint8_t *)"MCFI_GLPB", (const uint8_t *)"MCFI_GLPB" + 8);
    uint32_t cnt = (uint32_t)entries.size();
    out.insert(out.end(), (const uint8_t *)&cnt, (const uint8_t *)&cnt + 4);
    for (auto &kv : entries) {
        const ProgBinEntry &e = kv.second;
        out.insert(out.end(), (const uint8_t *)&e.hash, (const uint8_t *)&e.hash + 4);
        out.insert(out.end(), (const uint8_t *)&e.fmt, (const uint8_t *)&e.fmt + 4);
        out.insert(out.end(), (const uint8_t *)&e.len, (const uint8_t *)&e.len + 4);
...
static GLuint mcfi_link_compute(const char *cs_src) {
    // program binary 缓存：命中直接加载（驱动校验），未命中正常编译后回存
    static std::map<uint32_t, ProgBinEntry> g_pbc;
    static bool g_pbc_loaded = false;
    uint32_t h = gles_src_hash(cs_src);
    if (!g_pbc_loaded) {
        gles_prog_load(g_pbc);
        g_pbc_loaded = true;
    }
    auto it = g_pbc.find(h);
    if (it != g_pbc.end() && it->second.len > 0) {
        GLuint p = glCreateProgram();
        glProgramBinary(p, it->second.fmt, it->second.data.data(), (GLsizei)it->second.len);
        GLint ok = 0;
        glGetProgramiv(p, GL_LINK_STATUS, &ok);
        if (ok) {
            LOGI("GLES 程序命中缓存（hash=%08x）", h);
            return p;
        }
        LOGI("GLES program binary 失效（hash=%08x），重链", h);
        glDeleteProgram(p);
        g_pbc.erase(it);
    }
    ...
    glDeleteShader(cs);
    // 链接成功 → 回存二进制（驱动不提供/失败则静默跳过）
    GLint blen = 0;
    glGetProgramiv(p, GL_PROGRAM_BINARY_LENGTH, &blen);
    if (blen > 0 && blen <= (8 << 20)) {
        ProgBinEntry e;
        e.hash = h;
        e.len = (uint32_t)blen;
        e.data.resize((size_t)blen);
        GLenum fmt = 0;
        glGetProgramBinary(p, (GLsizei)blen, nullptr, &fmt, e.data.data());
        e.fmt = (uint32_t)fmt;
        if (e.fmt != 0 && !e.data.empty()) {
            g_pbc[h] = std::move(e);
            gles_prog_save(g_pbc);
        }
    }
    return p;
}
...
// T2-6：MV 场半精度。me_mv16=1 时 ME shader 的 MV image 声明 rgba32f -> rgba16f
//（mvTex 同步创建为 RGBA16F，带宽/存储减半；rgba16f 为 GLES3.1 必选 image 格式，
//  采样/上传仍走浮点路径，数值语义不变）。
static std::string me_src_mv16(const char *src) {
    if (g_cfg.me_mv16 == 0) return std::string(src);
    std::string s(src);
    size_t p = 0;
    while ((p = s.find("rgba32f", p)) != std::string::npos) { s.replace(p, 7, "rgba16f"); p += 7; }
    return s;
}

static GLuint link_me1(bool mv16 = false) {
    if (gles_subgroup_ok()) {
        GLuint p = mcfi_link_compute(mv16 ? me_src_mv16(CS_ME1_SUB_SRC).c_str() : CS_ME1_SUB_SRC);
        if (p) { LOGI("GLES ME1 已接入 subgroup 归约版"); return p; }
    }
    return mcfi_link_compute(mv16 ? me_src_mv16(CS_ME1_SRC).c_str() : CS_ME1_SRC);
}
...
static void make_tex(GLuint &tex, int w, int h, bool f32 = false, bool f16 = false) {
    glGenTextures(1, &tex);
    glBindTexture(GL_TEXTURE_2D, tex);
    // T2-6：f16=true 时 MV 场用 RGBA16F（半浮点，带宽/存储减半；rgba16f 为 GLES3.1 必选 image 格式）
    GLint ifmt = f16 ? GL_RGBA16F : (f32 ? GL_RGBA32F : GL_RGBA8);
    GLenum dfmt = f16 ? GL_HALF_FLOAT : (f32 ? GL_FLOAT : GL_UNSIGNED_BYTE);
    glTexImage2D(GL_TEXTURE_2D, 0, ifmt, w, h, 0, GL_RGBA, dfmt, nullptr);
...
// T2-6：MV 场 16bit 精度探测（默认关）。R16G16F storage 需 EXT_color_buffer_float；
// R16G16_UNORM 需 GL_EXT_texture_norm16。探测结果常驻日志（log_level>0），
// 供确定是否实施 16bit 数据路径；me_mv16 仅控制是否启用 16bit 路径。
static void gles_mv16_support() {
    const GLubyte *ext = glGetString(GL_EXTENSIONS);
    if (!ext) return;
    std::string s((const char *)ext);
    bool cb_float = s.find("GL_EXT_color_buffer_float") != std::string::npos;
    bool norm16   = s.find("GL_EXT_texture_norm16") != std::string::npos;
    bool hf       = s.find("GL_OES_texture_half_float") != std::string::npos;
    LOGI("MV16 探测: me_mv16=%d -> R16G16F(RG半浮点storage,需EXT_color_buffer_float)=%s, R16G16_UNORM(需GL_EXT_texture_norm16)=%s, 半浮点纹理=%s",
         g_cfg.me_mv16,
         cb_float ? "支持" : "不支持", norm16 ? "支持" : "不支持", hf ? "支持" : "不支持");
}
...
static GLuint run_motion_estimation(GLuint progME[2], GLuint progCopy, GLuint vao,
                                    ...
                                    GmvRes *gmv = nullptr, int lv2 = 4, bool mv16 = false) {
    ...
    glBindImageTexture(3, mvA, 0, GL_FALSE, 0, GL_WRITE_ONLY, mv16 ? GL_RGBA16F : GL_RGBA32F);
    ...
    if (progME[1]) {
        ...
        glBindImageTexture(4, mvB, 0, GL_FALSE, 0, GL_WRITE_ONLY, mv16 ? GL_RGBA16F : GL_RGBA32F);
...
static float run_motion_smooth_stats(GLuint progME[4], GLuint inMV, GLuint outMV,
                                     GLuint statsBuf, int mvW, int mvH, bool mv16 = false) {
    ...
    glBindImageTexture(1, outMV, 0, GL_FALSE, 0, GL_WRITE_ONLY, mv16 ? GL_RGBA16F : GL_RGBA32F);
    glDispatchCompute(gw, gh, 1);
    glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT | GL_TEXTURE_FETCH_BARRIER_BIT);
...
struct AsyncCtx {
    ...
    int nextFree = 0;
    bool meOk = true;
    bool mv16 = false;             // T2-6：MV 场半浮点（RGBA16F）实际生效标志（链接失败自动回退 rgba32f）
...
static void async_worker_main(AsyncCtx *a) {
    ...
    for (;;) {
        ...
        if (do_insert) {
            ...
            // 跳帧（game_interval>1 时的非插帧帧）：prevSlot 未被本帧 ME 读取，立即释放，
...
static bool async_init(AsyncCtx *a, EGLContext appCtx, int w, int h) {
    ...
    if (n <= 0) { LOGE("eglGetConfigs 返回 0 个 config"); return false; }
    ...
    // 回退路径：部分驱动（Mali/混合 EGL）的 eglGetConfigs 列表不含 app 上下文
    // 的 config（EGL_CONFIG_ID 是不透明值，厂商可不连续/不在枚举列表）。
    // 此时按"支持 ES3 渲染 + 颜色/深度位数最丰富"从列表挑一个兼容 config，
    // 用它创建与 app 共享的上下文（EGL 允许不同 config 的 context 共享）。
    if (!cfg) {
        EGLint best = -1;
        for (EGLConfig c : cfgs) {
            EGLint rt = 0;
            eglGetConfigAttrib(a->dpy, c, EGL_RENDERABLE_TYPE, &rt);
            if (!(rt & EGL_OPENGL_ES3_BIT)) continue;
            EGLint r = 0, g = 0, b = 0, al = 0, d = 0;
            eglGetConfigAttrib(a->dpy, c, EGL_RED_SIZE, &r);
            eglGetConfigAttrib(a->dpy, c, EGL_GREEN_SIZE, &g);
            eglGetConfigAttrib(a->dpy, c, EGL_BLUE_SIZE, &b);
            eglGetConfigAttrib(a->dpy, c, EGL_ALPHA_SIZE, &al);
            eglGetConfigAttrib(a->dpy, c, EGL_DEPTH_SIZE, &d);
            EGLint score = r + g + b + al + d;
            if (score > best) { best = score; cfg = c; }
        }
        if (cfg)
            LOGI("EGLConfig ID 匹配失败(%d)，按属性回退选择 ES3 config", (int)cfgId);
    }
    ...
    if (a->meOk) {
        // T2-6：me_mv16=1 时先用 rgba16f 版链接；任一失败回退 rgba32f 原版重链（mvTex 格式随 mv16 联动）
        a->mv16 = g_cfg.me_mv16 != 0;
        if (a->mv16) {
            a->progME[0] = mcfi_link_compute(me_src_mv16(CS_ME0_SRC).c_str());
            a->progME[1] = link_me1(true);
            a->progME[2] = mcfi_link_compute(me_src_mv16(CS_ME2_SRC).c_str());
            a->progME[3] = mcfi_link_compute(me_src_mv16(CS_ME3_SRC).c_str());
            if (!a->progME[0] || !a->progME[1] || !a->progME[2] || !a->progME[3]) {
                LOGE("rgba16f MV image 链接失败，回退 rgba32f（MV 场保持全精度浮点）");
                a->mv16 = false;
                for (int i = 0; i < 4; i++)
                    if (a->progME[i]) { glDeleteProgram(a->progME[i]); a->progME[i] = 0; }
                a->progME[0] = mcfi_link_compute(CS_ME0_SRC);
                a->progME[1] = link_me1(false);
    ...
    // rgba32f/rgba16f + writeonly image：绕开部分驱动不支持 read/write 混合访问的限制
    // （T2-6：me_mv16 生效时 MV 场用 RGBA16F 半浮点，带宽/存储减半）
    make_tex(a->mvTex[0], a->mvW2, a->mvH2, !a->mv16, a->mv16);   // mvA：L2 粗场
    make_tex(a->mvTex[1], a->mvW1, a->mvH1, !a->mv16, a->mv16);   // mvB：L1 场
    make_tex(a->mvTex[2], a->mvW1, a->mvH1, !a->mv16, a->mv16);   // mvC：L1 平滑输出（下一帧时间预测源）
...
static bool video_init(VideoCtx *v, EGLContext ctx, int w, int h) {
    ...
    if (v->meOk) {
        // T2-6：视频模式同 async 模式——me_mv16=1 时 rgba16f 链接，失败回退 rgba32f
        v->mv16 = g_cfg.me_mv16 != 0;
        if (v->mv16) {
            v->progME[0] = mcfi_link_compute(me_src_mv16(CS_ME0_SRC).c_str());
            v->progME[1] = link_me1(true);
            v->progME[2] = mcfi_link_compute(me_src_mv16(CS_ME2_SRC).c_str());
            v->progME[3] = mcfi_link_compute(me_src_mv16(CS_ME3_SRC).c_str());
            if (!v->progME[0] || !v->progME[1] || !v->progME[2] || !v->progME[3]) {
                LOGE("GLES 视频 rgba16f MV image 链接失败，回退 rgba32f");
                v->mv16 = false;
                for (int i = 0; i < 4; i++)
                    if (v->progME[i]) { glDeleteProgram(v->progME[i]); v->progME[i] = 0; }
                v->progME[0] = mcfi_link_compute(CS_ME0_SRC);
                v->progME[1] = link_me1(false);
    ...
    make_tex(v->mvTex[0], v->mvW2, v->mvH2, !v->mv16, v->mv16);
    make_tex(v->mvTex[1], v->mvW1, v->mvH1, !v->mv16, v->mv16);
    make_tex(v->mvTex[2], v->mvW1, v->mvH1, !v->mv16, v->mv16);
    {
...
static void video_interp_sync(EGLDisplay dpy, EGLSurface surf, int w, int h) {
    ...
    if (v->hasPrev) {
        ...
        if (v->meOk) {
            GLuint mvOut = run_motion_estimation(v->progME, v->progCopy, v->vao,
                                                 ...
                                                 range, refine, cfg.smooth / 100.0f, &v->gmv, 2, v->mv16);
            float lowRatio = run_motion_smooth_stats(v->progME, mvOut, v->mvTex[2],
                                                     v->statsBuf, v->mvW1, v->mvH1, v->mv16);
            if (lowRatio > 0.40f) imode = 1;
    ...
    // 4. 性能上限（同步耗时监控）：≥25ms 累计 10 次升一档（更宽松：短时卡顿/单帧波动不误升档）。
...
extern "C" void mcfi_payload_init(const char *config_text, int companion_fd) {
    ...
    if (want_vk) {
        mcfi_vk_install(cfg.log_level, g_backend, g_pkg.c_str());
    } else {
