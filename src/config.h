...
struct McfiConfig {
    ...
    int  game_interval = 1;        // 游戏插帧间隔：每 N 个真实帧插入 1 个生成帧（GLES/Vulkan 游戏模式）
    int  video_interval = 1;       // 视频插帧间隔：每 N 个真实帧插入 1 个生成帧（GLES/Vulkan 视频模式）
...
public:
    ...
    std::string dump() const {
        ...
        snprintf(buf, sizeof(buf),
                 ...
                 strength, smooth, game_interval, video_interval, me_quality, panel_port,
